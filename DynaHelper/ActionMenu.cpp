...
ActionMenu::ActionMenu(int indexAction, bool ableGoBack)
: _ableGoBack(ableGoBack),
  _goBack(false),
  _ctrlDown(false),
  ...
  _dropWindow(0)
{
	...
	initialize("Action Menu", "Action Menu", _globHINST, WS_EX_TOPMOST, (WS_OVERLAPPED | WS_SYSMENU | WS_MINIMIZEBOX), screenRect, NULL);
	...
	switch(indexAction) {
		...
		case TEXT_INP:		index = 7;									break;
		case K_TYPE:		index = 8;									break;
		case K_PRESS:		index = 9;									break;
		case REL_MOUSE_ACT:	index = 10;									break;
		case NO_ACT:		index = 11;									break;
		default:			cueBanner = new std::wstring(L"Actions");
	}
	std::vector<std::string> members(12);
	...
	members.at(7) = "Text Input";
	members.at(8) = "Key Type";
	members.at(9) = "Key Press";
	members.at(10) = "Relative Mouse Action";
	members.at(11) = "No Action";
...
LRESULT CALLBACK ActionMenu::wndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message) {
		case WM_LBUTTONUP:
			_dropWindow.setFocus();
		...
		case WM_PAINT:
			{
				...
				LinearGradientBrush gradBrush(Point(rcClient.left, rcClient.top), Point(rcClient.right, rcClient.bottom)
				  , Color(255, 255, 255), Color(100, 100, 255));
				...
				if(_ableGoBack) {
					graphics.DrawImage(&Image(L"BackSymbolPop.png", 0), 10, 62);
				}
				graphics.DrawImage(&Image(L"ForwardSymbolPop.png", 0), (rcClient.right - 30), 62);
		...
		case WM_KEYDOWN:
			if((wParam == VK_CONTROL) || (wParam == VK_BACK)) {
				...
				_goBack = (_ctrlDown && (wParam == VK_BACK) && _ableGoBack);
				if(_goBack) {
...
bool ActionMenu::isActionSelected() const
{
	return _selectionMade;
}


bool ActionMenu::wasBackEntered() const
{
	return _goBack;
}